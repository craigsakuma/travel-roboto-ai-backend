...
class Settings(BaseSettings):
    ...
    log_level: Literal["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"] = Field(
        default="DEBUG", 
        description="Python logging verbosity level."
    )
    ...
    openai_api_key: SecretStr | None = Field(
        default=None,
        description="API key for OpenAI (GPT-4, etc.)"
    )
    ...
    ab_testing_enabled: bool = Field(
        default=True,
        description="Enable A/B testing for models and prompts."
    )
    default_model_provider: Literal["openai", "anthropic", "google"] = Field(
        default="anthropic",
        description="Default LLM provider when A/B testing is disabled."
    )
    ...
    prompts_base_dir: Path = Field(
        default=BASE_DIR / "agents" / "prompts",
    ...
    gmail_webhook_secret: SecretStr | None = Field(
        default=None,
        description="Secret token for validating Gmail webhook requests."
    )
    ...
    gmail_client_secret: SecretStr | None = Field(
        default=None,
        description="Google OAuth 2.0 client secret for Gmail API access."
    )
